...
class Database extends Config
{
    ...
    public function __construct()
    {
        ...
        if (getenv('database.hris.hostname')) {
            $this->hris['hostname'] = getenv('database.hris.hostname');
            $this->hris['username'] = getenv('database.hris.username');
            $this->hris['password'] = getenv('database.hris.password');
            $this->hris['database'] = getenv('database.hris.database');
            $this->hris['DBDriver'] = getenv('database.hris.DBDriver') ?: 'MySQLi';
            $this->hris['port'] = (int) (getenv('database.hris.port') ?: 3306);
            $this->hris['charset'] = getenv('database.hris.charset') ?: 'utf8mb4';
            $this->hris['DBCollat'] = getenv('database.hris.DBCollat') ?: 'utf8mb4_general_ci';
        }
